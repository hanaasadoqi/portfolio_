    ...
    <div className="tabs">
      <ul className="hidden tab-list text-sm font-medium text-center text-gray-500 rounded-lg shadow sm:flex sm:justify-center sm:items-center dark:divide-gray-700 dark:text-gray-400">
        {children.map((tab, index) => (
          <li key={index}>
            <button
              className={clsx('p-0', { active: index === activeTab })}
      ...
      <div className="tab-content h-full mx-auto prose-xl flex flex-col items-center justify-center text-left px-4">
...
export const Tab: React.FC<TabProps> = ({ children }) => {
  return <div className="w-full min-w-[90vw] md:min-w-[50vw]">{children}</div>;
};

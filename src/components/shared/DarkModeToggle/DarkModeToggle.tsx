  ...
  return (
    <div id="dark-mode-toggle-container" className="fixed bottom-4 right-4">
      <motion.button
        ...
        className="flex h-12 w-12 items-center justify-center rounded-full bg-primary-700 p-2 shadow-md transition-colors duration-300 dark:bg-primary-100"
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 0.9 }}
        aria-label="Toggle dark mode"
      >
        {isDarkMode ? (
          <FaSun className="text-primary-900" />
        ) : (
          <FaMoon className="text-primary-100" />
        )}
      </motion.button>
